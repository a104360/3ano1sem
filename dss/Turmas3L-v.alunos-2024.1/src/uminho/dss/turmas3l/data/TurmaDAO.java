...
public class TurmaDAO implements Map<String, Turma> {
    ...
    public Set<String> keySet() {
        ...
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
             Statement stm = conn.createStatement()) {
            try(ResultSet rs = stm.executeQuery("SELECT Id FROM turmas;")){
                while(rs.next()){
    ...
    public Collection<Turma> values() {
        ...
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
             Statement stm = conn.createStatement();){ // ResultSet com os ids de todas as turmas
    ...
    public Set<Entry<String, Turma>> entrySet() {
        Set<Entry<String,Turma>> setEntry = new HashSet<>();
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
             Statement stm = conn.createStatement()) {
            try (ResultSet rsTurma = stm.executeQuery("SELECT * FROM turmas;")) {
                while(rsTurma.next()){
                    String id = rsTurma.getString("Id");


                    Sala sala = null;
                    try (ResultSet rsSala = stm.executeQuery("SELECT * FROM sala WHERE Num = '" + rsTurma.getString("Sala") + "';")) {
                        rsSala.next();
                        String numeroSala = rsSala.getString("Num");
                        String edificioSala = rsSala.getString("Edificio");
                        int capacidadeSala = rsSala.getInt("Capacidade");                       
                        sala = new Sala(numeroSala,edificioSala,capacidadeSala);
                    } catch (SQLException e) {
                        e.printStackTrace();
                        continue;
                    }
                    ...
                    try (ResultSet rsAlunos = stm.executeQuery("SELECT * FROM alunos WHERE Turma='" + id + "';")) {
                        while(rsAlunos.next()){
                            String numAluno = rsAlunos.getString("Num");
                            alunos.add(numAluno);
                        }
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                    Turma t = new Turma(id, sala, alunos);
                    setEntry.add(Map.entry(id,t));
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
